...
class LawnchairLauncher : QuickstepLauncher() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (launcher.isIconLabelsEnabled()) {
            launcher.enableIconLabels()
        } else {
            launcher.disableIconLabels()
        }
    ...
    private fun disableSwipeDown() {
        val prefs = PreferenceManager2.getInstance(this)
        prefs.swipeDownGestureHandler.setBlocking(GestureHandlerConfig.NoOp)
    }

    private fun enableSwipeDown() {
        val prefs = PreferenceManager2.getInstance(this)
        prefs.swipeDownGestureHandler.setBlocking(GestureHandlerConfig.OpenNotifications)
    }

    private fun disableSwipeGestures() {
        val prefs = PreferenceManager2.getInstance(this)
        prefs.swipeUpGestureHandler.setBlocking(GestureHandlerConfig.NoOp)
        prefs.swipeDownGestureHandler.setBlocking(GestureHandlerConfig.NoOp)
    }

    fun enableSwipeGestures() {
        val prefs = PreferenceManager2.getInstance(this)
        prefs.swipeUpGestureHandler.setBlocking(GestureHandlerConfig.OpenAppDrawer)
        prefs.swipeDownGestureHandler.setBlocking(GestureHandlerConfig.OpenNotifications)
    }
     ...
     * 启用主屏幕图标标签显示
     */
    private fun enableIconLabels() {
        // 获取偏好设置管理器
        val prefs = PreferenceManager2.getInstance(this)

        // 启用主屏幕图标标签
        prefs.showIconLabelsOnHomeScreen.setBlocking(true)
        // 启用文件夹中的图标标签
        prefs.showIconLabelsOnHomeScreenFolder.setBlocking(true)
        // 启用应用抽屉中的图标标签
        prefs.showIconLabelsInDrawer.setBlocking(true)
    }

    /**
     * 禁用主屏幕图标标签显示
     */
    private fun disableIconLabels() {
        // 获取偏好设置管理器
        val prefs = PreferenceManager2.getInstance(this)

        // 禁用主屏幕图标标签
        prefs.showIconLabelsOnHomeScreen.setBlocking(false)
        // 禁用文件夹中的图标标签
        prefs.showIconLabelsOnHomeScreenFolder.setBlocking(false)
        // 禁用应用抽屉中的图标标签
        prefs.showIconLabelsInDrawer.setBlocking(false)
    }

    /**
     * 检查图标标签显示状态
     * @return 如果启用了图标标签显示则返回 true，否则返回 false
     */
    private fun isIconLabelsEnabled(): Boolean {
        val prefs = PreferenceManager2.getInstance(this)
        prefs.showIconLabelsOnHomeScreen.setBlocking(true)
        return prefs.showIconLabelsOnHomeScreen.firstBlocking()
    }
