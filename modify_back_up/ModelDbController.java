...
public class ModelDbController {
    ...
    public synchronized void loadDefaultFavoritesIfNecessary() {
        ...
        ResolveInfo defaultBrowser = pm.resolveActivity(browserIntent, PackageManager.MATCH_DEFAULT_ONLY);

        // 如果找到默认浏览器，添加到主屏幕
        if (defaultBrowser != null) {
            ContentValues values = new ContentValues();
            values.put(LauncherSettings.Favorites.CONTAINER, LauncherSettings.Favorites.CONTAINER_DESKTOP);
            values.put(LauncherSettings.Favorites.SCREEN, 0);    // 第一个屏幕
            values.put(LauncherSettings.Favorites.CELLX, 0);     // X 坐标
            values.put(LauncherSettings.Favorites.CELLY, 0);     // Y 坐标
            values.put(LauncherSettings.Favorites.ITEM_TYPE, LauncherSettings.Favorites.ITEM_TYPE_APPLICATION);

            Intent launchIntent = new Intent(Intent.ACTION_MAIN);
            launchIntent.addCategory(Intent.CATEGORY_LAUNCHER);
            launchIntent.setPackage(defaultBrowser.activityInfo.packageName);
            launchIntent.setComponent(new ComponentName(
                defaultBrowser.activityInfo.packageName,
                defaultBrowser.activityInfo.name));

            values.put(LauncherSettings.Favorites.INTENT, launchIntent.toUri(0));

            values.put(LauncherSettings.Favorites.TITLE, defaultBrowser.loadLabel(pm).toString());
            /*db.insert(LauncherSettings.Favorites.TABLE_NAME, null, values);*/
        }

        // 2. 检查并添加 com.arglasses.launcher
        try {
            PackageInfo arglassesInfo = pm.getPackageInfo("com.arglasses.launcher", 0);
            if (arglassesInfo != null) {
                // 获取启动 Activity
                Intent launchIntent = pm.getLaunchIntentForPackage("com.arglasses.launcher");
                if (launchIntent != null) {
                    ContentValues values = new ContentValues();
                    values.put(LauncherSettings.Favorites.CONTAINER, LauncherSettings.Favorites.CONTAINER_DESKTOP);
                    values.put(LauncherSettings.Favorites.SCREEN, 0);    // 第一个屏幕
                    values.put(LauncherSettings.Favorites.CELLX, 0);     // X 坐标 (放在浏览器右边)
                    values.put(LauncherSettings.Favorites.CELLY, 0);     // Y 坐标
                    values.put(LauncherSettings.Favorites.ITEM_TYPE, LauncherSettings.Favorites.ITEM_TYPE_APPLICATION);
                    values.put(LauncherSettings.Favorites.INTENT, launchIntent.toUri(0));

                    values.put(LauncherSettings.Favorites.TITLE, pm.getApplicationLabel(Objects.requireNonNull(arglassesInfo.applicationInfo)).toString());
                    db.insert(LauncherSettings.Favorites.TABLE_NAME, null, values);
                }
            }
        ...
        try {
            PackageInfo arglassesInfo = pm.getPackageInfo("com.xiaodu.dueros.magictool.demo", 0);
            if (arglassesInfo != null) {
                // 获取启动 Activity
                Intent launchIntent = pm.getLaunchIntentForPackage("com.xiaodu.dueros.magictool.demo");
                if (launchIntent != null) {
                    ContentValues values = new ContentValues();
                    values.put(LauncherSettings.Favorites.CONTAINER, LauncherSettings.Favorites.CONTAINER_DESKTOP);
                    values.put(LauncherSettings.Favorites.SCREEN, 0);    // 第一个屏幕
                    values.put(LauncherSettings.Favorites.CELLX, 1);     // X 坐标 (放在浏览器右边)
                    values.put(LauncherSettings.Favorites.CELLY, 0);     // Y 坐标
                    values.put(LauncherSettings.Favorites.ITEM_TYPE, LauncherSettings.Favorites.ITEM_TYPE_APPLICATION);
                    values.put(LauncherSettings.Favorites.INTENT, launchIntent.toUri(0));
